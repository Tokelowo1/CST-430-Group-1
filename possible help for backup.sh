...
rsync -avz --exclude='lost+found' /data/ \
    "$backupusername@$backupserverip:$finaldest/data_file"
if [ $? -ne 0 ]; then
...
if [ $? -ne 0 ]; then
    echo "Cannot create config folder"
    exit 1
fi

rsync -avz /etc/fstab /etc/ssh/sshd_config \
    "$backupusername@$backupserverip:$finaldest/configs/"
if [ $? -ne 0 ]; then
